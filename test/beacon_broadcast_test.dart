...
void main() {
  ...
  group('starting beacon advertising', () {
    ...
    test('when uuid is not set throws exception', () async {
      onStartMethodReturn(Future.value());
      expect(() => BeaconBroadcast().setMajorId(1).setMinorId(1).setTransmissionPower(-59).setIdentifier("identifier").start(),
          throwsA(isA<IllegalArgumentException>()));
    });

    test('when major id is not set throws exception', () async {
      onStartMethodReturn(Future.value());
      expect(() => BeaconBroadcast().setUUID("uuid").setMinorId(1).setTransmissionPower(-59).setIdentifier("identifier").start(),
          throwsA(isA<IllegalArgumentException>()));
    });

    test('when minor id is not set throws exception', () async {
      onStartMethodReturn(Future.value());
      expect(() => BeaconBroadcast().setUUID("uuid").setMajorId(1).setTransmissionPower(-59).setIdentifier("identifier").start(),
          throwsA(isA<IllegalArgumentException>()));
    });
    ...
    test('when extra data contains integer out of range throws exception', () async {
      onStartMethodReturn(Future.value());
      expect(() => BeaconBroadcast().setUUID("uuid").setExtraData([270]).start(), throwsA(isA<IllegalArgumentException>()));
    });
    ...
    group('when custom layout is set', () {
      test('and minor id is not set returns normally', () async {
        onStartMethodReturn(Future.value());
        expect(
            () => BeaconBroadcast().setUUID("uuid").setMajorId(1).setTransmissionPower(-59).setIdentifier("identifier").setLayout("layout").start(),
            returnsNormally);
      });
      ...
      test('and major id is not set returns normally', () async {
        onStartMethodReturn(Future.value());
        expect(
            () => BeaconBroadcast().setUUID("uuid").setMinorId(1).setTransmissionPower(-59).setIdentifier("identifier").setLayout("layout").start(),
            returnsNormally);
      });

      test('and UUID is not set throws exception', () async {
        onStartMethodReturn(Future.value());
        expect(() => BeaconBroadcast().setMinorId(1).setTransmissionPower(-59).setIdentifier("identifier").setLayout("layout").start(),
            throwsA(isA<IllegalArgumentException>()));
      });
    });
  });
